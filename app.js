...
const TEAM_MEMBERS = [
  ...
  { id: 'elvis', name: 'Elvis Falcão', role: 'Content Lead' },
...
function memberOptions(selected = '') { return `<option value="">Unassigned</option>${state.members.map(member => `<option value="${member.id}" ${member.id === selected ? 'selected' : ''}>${escape(member.name)} — ${escape(member.role)}</option>`).join('')}`; }
...
function renderOverview() {
  ...
  document.getElementById('priorityTasks').innerHTML = priority.length ? priority.map(taskCard).join('') : `<div class="task-card"><div class="task-mark"></div><div><h3>You’re clear</h3><p>Import a budget plan or load the sample campaign to see operations here.</p></div></div>`;
...
function taskCard(t) { const f = taskStatus(t); const dueClass = f.overdue ? 'overdue' : ''; const context = [t.campaignName || (t.campaignId ? campaignName(t.campaignId) : ''), t.market, t.platform].filter(Boolean).join(' · ') || 'General task'; return `<article class="task-card" data-task-id="${t.id}"><div class="task-mark ${t.type}"></div><div><h3>${escape(t.title)}</h3><p>${escape(context)} · <span class="badge ${t.type}">${t.type || 'To-do'}</span> · ${escape(memberName(t.assigneeId))}</p></div><div class="due ${dueClass}">${f.overdue ? 'Overdue · ' : ''}${escape(t.dueDate)}<br><span class="badge">${escape(t.status)}</span></div></article>`; }
...
function renderCampaigns() { document.getElementById('campaignList').innerHTML = state.campaigns.map(c => { const tasks = state.tasks.filter(t => t.campaignId === c.id); return `<article class="campaign"><p class="eyebrow">${escape(BRAND_CATALOG[c.brand]?.division || '')} · ${escape(c.market)}</p><h3>${escape(c.name)}</h3><p>${escape(c.brand)} · ${tasks.length} generated operational tasks</p><div class="meta"><span>FluxPlanner: ${c.fluxPlanId ? 'linked' : 'spreadsheet import'}</span><span>${tasks.filter(t => t.status === 'Done').length}/${tasks.length} done</span></div></article>`; }).join('') || '<p>No campaigns have been imported yet.</p>'; }
function renderReports() { const reports = state.tasks.filter(t => t.type === 'Report').sort((a,b) => a.dueDate.localeCompare(b.dueDate)); document.getElementById('reportList').innerHTML = reports.map(t => { const f = taskStatus(t); return `<article class="task-card" data-task-id="${t.id}"><div class="task-mark Report"></div><div><h3>${escape(t.title)}</h3><p>${escape(t.objective || 'Objective not set')} · Budget ${t.budget || 0} · ${t.reportState || 'Awaiting data'}</p></div><div class="due ${f.reportDue ? 'overdue' : ''}">${f.reportDue ? 'Due · ' : ''}${t.dueDate}</div></article>`; }).join('') || '<p>No reporting tasks yet.</p>'; bindTaskCards(); }
function renderSettings() { document.getElementById('settingsPanel').innerHTML = `<section class="setting"><h3>Role templates</h3><p>Workflow tasks are assigned by role so the workspace admin can map people without changing the process.</p><ul>${WORKFLOW_TEMPLATE.map(s => `<li>${s.order}. ${s.name} → ${s.role}</li>`).join('')}</ul></section><section class="setting"><h3>Notification channels</h3><p><b>In-app:</b> active<br><b>Email:</b> configure Resend secret + verified sender<br><b>Teams:</b> inactive until IT approves and configures a scoped channel integration.</p></section><section class="setting"><h3>Supermetrics reporting</h3><p>Configure API key and saved-query mappings as Supabase Edge Function secrets. Failed requests leave report tasks available for manual entry.</p></section><section class="setting"><h3>FluxPlanner sync</h3><p>Supabase-only plans. Structural campaign changes originate in FluxPlanner; task state and links stay here. Date collisions are recorded for review.</p></section>`; }
