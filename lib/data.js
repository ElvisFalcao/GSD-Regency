...
export function createDataLayer(client, { workspaceId }) {
  ...
  // Puts a creative into the public bucket and returns its URL — which is how
  // the Graph API fetches it. Free tier caps files at ~50MB; big videos should
  // be pasted as a direct URL instead.
  async function uploadCreative(file, taskId) {
    const safeName = file.name.replace(/[^a-zA-Z0-9.\-_]/g, '_');
    const path = `${taskId}/${Date.now()}-${safeName}`;
    const { error } = await client.storage.from('creatives').upload(path, file, { upsert: false });
    fail(error, 'Could not upload the creative.');
    const { data } = client.storage.from('creatives').getPublicUrl(path);
    return data.publicUrl;
  }

  // Publishes a Post task through meta-publish; the page token never leaves
  // the server. Returns { id, url, page } and the function has already marked
  // the task Done with the live link.
  async function publishTask({ taskId, pageId, message, link, mediaUrl, mediaType }, supabaseUrl, anonKey) {
    const { data: { session } } = await client.auth.getSession();
    if (!session) throw new Error('Sign in first.');
    const response = await fetch(`${supabaseUrl}/functions/v1/meta-publish`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${session.access_token}`, apikey: anonKey, 'Content-Type': 'application/json' },
      body: JSON.stringify({ taskId, pageId, message, link: link || undefined, mediaUrl: mediaUrl || undefined, mediaType: mediaType || undefined })
    });
  ...
  return {
    ...
    linkMember, approveNewMember, setAccessLevel, grantRole, revokeRole, removeRequest, listConnections, removeConnection, startMetaConnect, publishTask, uploadCreative,
    confirmAssignments
  };
}
