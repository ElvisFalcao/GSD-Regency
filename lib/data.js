...
export function createDataLayer(client, { workspaceId }) {
  ...
  async function listPublishedPlans() {
    const { data, error } = await client.from('plans')
      .select('id, campaign_name, country, total_budget, owner_email, updated_at, published_at, data')
  ...
  return {
    currentMember, requestAccess, listMembers, listCampaigns, listPublishedPlans, listTasks,
    createTask, updateTask, setFinancials, logActivity, importCampaign,
    linkMember, approveNewMember, setAccessLevel, grantRole, revokeRole, removeRequest,
    confirmAssignments
  };
}
