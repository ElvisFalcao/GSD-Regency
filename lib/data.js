...
export function createDataLayer(client, { workspaceId }) {
  ...
  async function publishTask({ taskId, pageId, message, link }, supabaseUrl, anonKey) {
    ...
    const response = await fetch(`${supabaseUrl}/functions/v1/meta-publish`, {
      ...
      body: JSON.stringify({ taskId, pageId, message, link: link || undefined })
    });
  ...
  return {
    ...
    linkMember, approveNewMember, setAccessLevel, grantRole, revokeRole, removeRequest, listConnections, removeConnection, startMetaConnect, publishTask,
    confirmAssignments
  };
}
